		...
		if (typeof server.players[message.author.id].tictactoe === 'undefined') {
			...
			if (message.mentions.members.size > 0) {
				...
				if (challenged.id === global.bot.user.id || challenged.id === message.author.id) {
					...
					if (collected.size < 1)
						return message.channel.send('Timed out. Type "cancel" to cancel this game and then type .ttt to start a new one.').catch(global.logger.error);
			...
			message.channel.send('You are already in a game! Type .ttt v to resend the grid.').catch(global.logger.error);
		}
	}
};